...
this->Log(GLLogStream::FILTER, "pickmode: %i", pickmode);
		if (pickmode == 0)
		{
			NewFaceSel.clear();
this->Log(GLLogStream::FILTER, "gla->height(): %d", gla->height());
this->Log(GLLogStream::FILTER, "cur mouse location x,y2: %d, %d", cur.x(), gla->height() - cur.y());
